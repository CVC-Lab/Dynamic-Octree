## TODO: 1. optimize the nb_lists_dist and calculate only for needed objects
##       2. deal with cases where the objects go out of the root bounding box

import sys, math, random
from objects import Object   # Import the Object class from the objects module
import numpy as np

INIT_NUM_OCTREE_NODES = 4  # Initial number of octree nodes
...
class DynamicOctree:
    ...
    def print_children(self, node_id):
        ...
        children = [i for i, node in enumerate(self.nodes) if node.parent_pointer == node_id]
        if self.verbose:
...
#---------------BUILD OCTREE---------------
    ...
    def init_free_node_server(self):
        ...
        # for i in range(self.num_nodes):
            ...
            # self.nodes[i].set_parent_pointer(-1)
...
#---------------UPDATION---------------
    ...
    def update_octree(self, atom, new_position):
        ...
        previous_node = self.nodes[self.object_to_node_map[atom]]
        ...
        # Early exit if the atom is still within the same node
        if self.inside_node(previous_node, atom):
            if self.verbose:
                print("No need to update octree as the atom stays in the current node.")
            # Update the neighborhood lists locally if needed
            self.update_nb_lists(self.atoms.index(atom), previous_node)
            return
        
        # Step 3: Remove the atom from the current node
        self.remove_point_from_node(current_node, atom)
        
       # Step 4: Find the root of the local subtree (T_i) containing the atom's new position
    ...
    def update_nb_lists_local(self, atom_index, target_node):
        ...
        self.update_nb_lists(atom_index, target_node, local_only=True)
