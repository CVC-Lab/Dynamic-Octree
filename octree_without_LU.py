...
class DynamicOctree:
    def __init__(self, atoms, n_atoms, cons_par, verbose=True, max_nodes=None, interaction_distance=10):
        """
...
#---------------BUILD OCTREE---------------
    ...
    def compute_root_bounding_box(self, node_id, slack_factor, indices, start_id, end_id):
        ...
        for i in range(start_id + 1, end_id + 1):
            ...
            maxX = 600 # max(maxX, self.atoms[j].getX())

            minY = -50 # min(minY, self.atoms[j].getY())
            maxY = 600 # max(maxY, self.atoms[j].getY())

            minZ = -50 # min(minZ, self.atoms[j].getZ())
            maxZ = 600 # max(maxZ, self.atoms[j].getZ())
